...
void fire_tracer(edict_t* self, vec3_t start, vec3_t dir, int damage, int mod)
{
	...
	/* int effect = EF_HYPERBLASTER; MetalGod initialized, but unused! */
	edict_t* bolt;
	trace_t	tr;
...
void Shrapnel_Explode(edict_t* ent)
{
	...
	if (ent->obj_owner == 0)
	{
		int j; /* MetalGod declariation of i removed, as it hid the prior local declaration */
...
void fire_rocket(edict_t* self, vec3_t start, vec3_t dir, int damage, int speed, float damage_radius, int radius_damage)
{
	...
	{
		rocket->s.effects |= EF_GRENADE;
		rocket->nextthink /* MetalGod = rocket->nextthink */ = level.time + 8000 / speed;
		rocket->think = G_FreeEdict;
	}
...
//void fire_rifle (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int kick, int mod)
{
	vec3_t		from;
	vec3_t		end;
	trace_t		tr = { 0 }; /* MetalGod initialized */
	edict_t* ignore;
	int			mask;
	qboolean	water;

	int     soundtype = 0;/* MetalGod initialized */
	int		ric_chance;
...
void Weapon_Rocket_Fire(edict_t* ent)
{
	...
	GunInfo_t* guninfo = ent->client->pers.weapon->guninfo;
	int mag_index = ent->client->pers.weapon->mag_index;
	/*int mod = guninfo->MeansOfDeath; MetalGod initialized, but not referenced */
	/*int	radius_damage = guninfo->damage_radius; //The *damage* within the radius MetalGod initialized, but not referenced */
	int	damage = guninfo->damage_direct;
...
void TNT_RadiusDamage(edict_t* inflictor, edict_t* attacker, float damage, edict_t* ignore, float radius, int mod)
{
	...
	while ((ent = findradius(ent, inflictor->s.origin, radius)) != NULL)
	{
		...
		knockb = .3F * (radius - distance); /* MetalGod explicit float */
...
void Weapon_Molotov_Fire(edict_t* ent)
{
	...
	/* int mag_index = ent->client->pers.weapon->mag_index; MetalGod initialized, but not referenced */
